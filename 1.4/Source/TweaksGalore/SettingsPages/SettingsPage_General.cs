...
namespace TweaksGalore
{
    public static class SettingsPage_General
    {
        ...
        {
            string categoryString = "Cat_General_Main";
            bool categoryToggle = mod.GetCollapsedCategoryState(categoryString);
            listing.LabelBackedHeader("General", mod.headerColor, ref categoryToggle);
            ...
            if (!categoryToggle)
            {
                if (listing.ButtonTextLabeled("", "Restore defaults"))
                {
                    ...
                    Messages.Message("Tweaks Galore: 'General' tweaks restored to defaults. Restart required to take effect.", MessageTypeDefOf.CautionInput);
                    TweaksGaloreMod.mod.restoreGeneral = true;
                }
                if (mod.restoreGeneral)
                {
                ...
                else
                {
                    ...
                    //listing.CheckboxEnhanced("Disable Narrow Heads", "Removes the narrow head shapes, they do not fit many of even the vanilla hair and beard styles properly and thanks to 1.4 making them defs it is very easy to just YEET. Can cause errors due to mods commonly directly referencing the removed head types.", ref settings.tweak_disableNarrowHeads);
                    //listing.GapLine();
                    ...
                    //}
                    // Tweak: Skill Per Day
                    //listing.DoSettingInt("Skill Learning Per Day", "After a set amount of xp has been learn for a skill since the last midnight passes, the amount that can be learnt is lowered. This lets you set how much that xp value is.", "tweak_skillLearningPerDay", 4000, 0, 20000);
                    //listing.GapLine();
                    // Tweak: Slim Rim
                    //listing.LabelBacked("Slim Rim", Color.white);
                    //listing.Note("Allows the disabling of body types on pawn generation. Does this during generation of the pawn so will not affect existing ones. The replacement body will be either Male or Female, whichever matches the pawns gender.", GameFont.Tiny);
                    //listing.CheckboxLabeled("Enabled", ref settings.patch_slimRim);
                    ...
                    //listing.LabelBacked("Trait Count Adjustment", Color.white);
                    //listing.Note("As of 1.4 this isn't fully functional, changes to the way traits generate made it so the first 1-3 can always generate like normal, regardless of what I do. So this is only usefuly currently for those who want to guarantee more than 1 or have more in general. Won't affect newborns/children as they have a complicated system for their growth.", GameFont.Tiny);
                    //listing.CheckboxLabeled("Enabled", ref settings.tweak_traitCountAdjustment);
