...
namespace TweaksGalore
{
    public static class SettingsPage_Polux
    {
        ...
        {
            string categoryString = "Cat_Biotech_Polux";
            bool categoryToggle = mod.GetCollapsedCategoryState(categoryString);
            listing.LabelBackedHeader("Polux Tree", mod.headerColor, ref categoryToggle);
            ...
            if (!categoryToggle)
            {
                if (listing.ButtonTextLabeled("", "Restore defaults"))
                {
                    ...
                    Messages.Message("Tweaks Galore: 'Polux Tree' tweaks restored to defaults. Restart required to take effect.", MessageTypeDefOf.CautionInput);
