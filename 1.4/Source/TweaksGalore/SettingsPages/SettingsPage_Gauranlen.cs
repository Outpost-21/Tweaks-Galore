...
namespace TweaksGalore
{
    public static class SettingsPage_Gauranlen
    {
        ...
        {
            string categoryString = "Cat_Ideology_Gauranlen";
            bool categoryToggle = mod.GetCollapsedCategoryState(categoryString);
            listing.LabelBackedHeader("Gauranlen Tree", mod.headerColor, ref categoryToggle);
            ...
            if (!categoryToggle)
            {
                if (listing.ButtonTextLabeled("", "Restore defaults"))
                {
                    ...
                    Messages.Message("Tweaks Galore: 'Gauranlen Tree' tweaks restored to defaults. Restart required to take effect.", MessageTypeDefOf.CautionInput);
                    TweaksGaloreMod.mod.restoreGauranlen = true;
                }
                if (mod.restoreGauranlen)
                {
