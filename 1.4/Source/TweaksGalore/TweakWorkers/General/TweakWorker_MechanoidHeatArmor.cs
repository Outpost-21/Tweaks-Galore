            ...
            if (!GetAllMechanoids.EnumerableNullOrEmpty())
            {
                ...
                foreach (ThingDef mechDef in GetAllMechanoids)
                {
                    if (mechDef != null & !mechDef.defName.Contains("Corpse"))
                    {
                        ...
                        {
                            LogUtil.LogMessage("Mechanoid Recognised: " + mechDef.defName + "/" + mechDef.label + ", but not patched as it has no heat armour value. This is intended behaviour not an error.");
                        }
                    }
                }
            }
