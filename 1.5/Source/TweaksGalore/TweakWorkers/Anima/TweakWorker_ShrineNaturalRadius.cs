...
{
    public class TweakWorker_ShrineNaturalRadius : TweakWorker
    {
        ...
        public override void OnStartup()
        {
            if (TGTweakDefOf.Tweak_AnimaTweaks.BoolValue)
            {
                foreach (ThingDef shrine in def.tweakThings)
                {
                    ...
                    FocusStrengthOffset_BuildingDefs shrineNatureRange = (FocusStrengthOffset_BuildingDefs)shrineFocus.offsets.Find(os => os.GetType() == typeof(FocusStrengthOffset_BuildingDefs));

                    shrineNatureRange.radius = def.FloatValue;
                }
            }
        }
